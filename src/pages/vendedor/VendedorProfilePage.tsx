...
export function VendedorProfilePage() {
  ...
  async function handleLogoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (error) { toast(t('vendor.logoError'), 'error'); setUploading(false); return }
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-5">
      <h2 className="font-display text-xl font-semibold text-gray-900 mb-5">{t('vendor.myProfile')}</h2>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-2xl border border-sand-200 p-5 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm font-semibold text-gray-700 font-body">{t('vendor.description')}</label>
          <textarea
            rows={3}
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm font-semibold text-gray-700 font-body">{t('vendor.category')}</label>
          <select
