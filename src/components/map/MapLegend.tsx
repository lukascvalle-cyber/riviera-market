...
export function MapLegend() {
  ...
  return (
    <div className="absolute bottom-6 left-4 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-3 flex flex-col gap-1.5 z-10">
      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide font-body mb-0.5">{t('map.categories')}</p>
      {CATEGORIES.map(cat => (
        <div key={cat} className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-700 font-body">{t(`categories.${cat}`)}</span>
        </div>
      ))}
    </div>
  )
